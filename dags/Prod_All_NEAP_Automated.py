...
def CreateJoinTableOfNeap():
    ...
    with sshtunnel.SSHTunnelForwarder(
        ...
        df3 = client.query_dataframe(
                        "SELECT * FROM test.xbi_neap WHERE Yr = 2022 and ActivityID IN (2,10)")
        if df3.empty:
            client.insert_dataframe(
                'INSERT INTO test.xbi_neap VALUES', dff)
            logger.info(
            ...
            sql = "ALTER TABLE test.xbi_neap DELETE WHERE Yr = 2022 and ActivityID IN (2,10)"
            client.execute(sql)
            client.insert_dataframe(
                'INSERT INTO test.xbi_neap VALUES', dff)
            logger.info(
