    ...
    with sshtunnel.SSHTunnelForwarder(
        ...
        for c, y in zip(campIDold_list, campYr_list):
            ...
            if data['data'] == "No data found":
                print("No Data found for campaign: "+str(c))
                logger.info('No Data found for campaign: '+str(c))
            else:
    ...
    with sshtunnel.SSHTunnelForwarder(
        ...
        for f, b in zip(campIDold_list, campYr_list):
            ...
            if apiDataFrame.empty:
                ...
                data_q = "SELECT * FROM test.xbi_after_catchUp WHERE Yr = "+str(b)+" AND ActivityID = "+str(f)
                df2 = client.query_dataframe(data_q) 
                #df2 = client.query_dataframe("SELECT * FROM test.xbi_after_catchUp WHERE Yr = 2022 and ActivityID = 2")
                if df2.empty:
                    client.insert_dataframe(
                    ...
                    sql = "ALTER TABLE test.xbi_after_catchUp DELETE WHERE Yr = "+ str(b) + " AND ActivityID = "+str(f)
                    client.execute(sql)
...
#                 sql = "SELECT " + cols + "  FROM test.get_after_catch_up eoc_1 left JOIN test.eoc_geolocation_tbl eoc_2 ON eoc_1.location_code  = eoc_2.code JOIN test.xbi_campaign eoc_3 ON (eoc_1.ActivityID  = eoc_3.campaign_ActivityID_old And eoc_1.Yr = eoc_3.campaign_Yr) "
