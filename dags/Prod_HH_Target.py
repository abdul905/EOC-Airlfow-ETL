    ...
    with sshtunnel.SSHTunnelForwarder(
        ...
        for f, b in zip(campIDold_list, campYr_list):
            ...
            if apiDataFrame.empty:
                ...
                print("Data found for campaign: "+str(f))
                ...
                df2 = client.query_dataframe("SELECT * FROM test.xbi_hhtarget WHERE Yr = "+str(b)+" AND ActivityID_fk = "+str(f)) 
                print("API DF Size: ",len(df2))       
                # df2 = client.query_dataframe(
                #     "SELECT * FROM test.xbi_hhtarget WHERE Yr = 2022 and ActivityID_fk IN ("+",".join((str(n) for n in li1)) + ")")   
                if df2.empty:
                    ...
                    sql = "ALTER TABLE test.xbi_hhtarget DELETE WHERE Yr = "+ str(b) + " AND ActivityID_fk = "+str(f)
                    # sql = "ALTER TABLE test.xbi_hhtarget DELETE WHERE Yr = 2022 and ActivityID_fk IN ("+",".join((str(n) for n in li1)) + ")"
                    client.execute(sql)
...
#                     sql = "SELECT " + cols + "  FROM test.get_hhtarget eoc_1 left JOIN test.eoc_geolocation_tbl eoc_2 ON eoc_1.Ucode  = eoc_2.code left JOIN test.xbi_campaign eoc_3 ON (eoc_1.ActivityID_fk  = eoc_3.campaign_ActivityID_old And eoc_1.Yr = eoc_3.campaign_Yr) "
#                     data = client.execute(sql)
